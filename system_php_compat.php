...
<title>Compatibilidade PHP do Sistema</title>
<style>
  :root{
    --bg:#f3f7ff;
    --panel:#ffffff;
...
<body>
  <div class="wrap">
    <div class="head">
      <div>
        <h1 class="title">Compatibilidade PHP</h1>
        <div class="sub">Validacao de extensoes necessarias e diretivas php.ini por ambiente da VM.</div>
      </div>
    ...
    <section class="panel">
      <div class="req-grid">
        <div class="req-box">
          <div class="req-title">Extensoes Necessarias</div>
          <ul id="req-required-ext" class="req-list"><li class="req-empty">Sem requisitos.</li></ul>
        </div>
        <div class="req-box">
          <div class="req-title">Diretivas php.ini requeridas</div>
          <ul id="req-ini" class="req-list"><li class="req-empty">Sem requisitos.</li></ul>
        </div>
      </div>
    </section>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>VM</th>
              <th>Status</th>
              <th>Extensoes faltantes</th>
              <th>Diretivas nao atendidas</th>
              <th>Observacoes</th>
            </tr>
          </thead>
          <tbody id="env-body">
            <tr><td colspan="6" class="req-empty">Carregando...</td></tr>
          </tbody>
        </table>
      </div>
    </section>
...
function requirementListFromSystem(system, keyList, keyRaw, splitFn){
  const list = Array.isArray(system?.[keyList]) ? system[keyList] : [];
  if (list.length) return list.map((item) => String(item || '').trim()).filter(Boolean);
  return splitFn(system?.[keyRaw] || '');
}

function renderReqList(targetId, items, formatter=null){
  ...
  el.innerHTML = items.map((item) => {
    const content = formatter ? formatter(item) : `<span class="req-code">${escHtml(item)}</span>`;
...
function renderEnvironmentRows(compat){
  ...
  if (!envs.length) {
    body.innerHTML = '<tr><td colspan="6" class="req-empty">Sem validacoes de ambiente.</td></tr>';
    return;
  }
  body.innerHTML = envs.map((env) => {
    ...
    const missingRequired = Array.isArray(env?.missing_required_extensions) ? env.missing_required_extensions : [];
    const unmetIni = Array.isArray(env?.unmet_ini_rules) ? env.unmet_ini_rules : [];
    const notes = Array.isArray(env?.notes) ? env.notes : [];

    const unmetIniHtml = unmetIni.length
      ? `<ul class="req-list">${unmetIni.map((rule) => {
        const directive = String(rule?.directive || '').trim();
        const operator = String(rule?.operator || '').trim();
        const expected = String(rule?.expected || '').trim();
        const actual = String(rule?.actual || '').trim();
        const expression = directive
          ? `${directive}${operator && operator !== 'exists' ? ` ${operator} ${expected}` : ''}`
          : String(rule?.raw || '').trim();
        return `<li><code>${escHtml(expression || String(rule?.raw || '-'))}</code>${actual !== '' ? ` (atual: <code>${escHtml(actual)}</code>)` : ''}</li>`;
      }).join('')}</ul>`
    ...
    return `
      <tr>
        ...
        <td>${missingRequired.length ? `<ul class="req-list">${missingRequired.map((item) => `<li><code>${escHtml(item)}</code></li>`).join('')}</ul>` : '<span class="req-empty">-</span>'}</td>
        <td>${unmetIniHtml}</td>
        <td>${notes.length ? `<ul class="req-list">${notes.map((note) => `<li>${escHtml(note)}</li>`).join('')}</ul>` : '<span class="req-empty">-</span>'}</td>
      </tr>
    `;
  }).join('');
}
...
async function loadSystemCompatibility(){
  ...
  try {
    const system = await apiCall(`system-php-compat-get&id=${encodeURIComponent(String(SYSTEM_ID))}`);
    ...
    const compat = system?.php_compatibility || {};
    ...
    const requiredExt = requirementListFromSystem(system, 'php_required_extensions_list', 'php_required_extensions', splitCsv);
    const iniRules = requirementListFromSystem(system, 'php_required_ini_list', 'php_required_ini', splitLines);

    renderReqList('req-required-ext', requiredExt);
    renderReqList('req-ini', iniRules);
    ...
    if (envBody) envBody.innerHTML = '<tr><td colspan="6" class="req-empty">Falha ao carregar dados.</td></tr>';
  }
}
